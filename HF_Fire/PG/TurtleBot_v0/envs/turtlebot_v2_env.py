...
class TurtleBotV2Env(gym.Env):
	...
	def __init__(self, map_width=None, map_height=None, items_id=None, items_quantity=None, initial_inventory = None, is_fire = None, goal_env = None, is_final = False):
		...
		self.object_types = [0,1,2,3,4] # we have 4 objects: wall, tree, rock, craft table and fire
	...
	def reset(self):
		...
		x_rand = np.random.rand(self.n_trees + self.n_rocks + self.n_table + self.is_fire, 1)
		y_rand = np.random.rand(self.n_trees + self.n_rocks + self.n_table + self.is_fire, 1)
		self.x_pos = []
		self.y_pos = []
		...
		for i in range(self.is_fire):
			if abs(-self.width/2+self.width*x_rand[i + self.n_trees + self.n_rocks + self.n_table]) < 0.3 and abs(-self.height/2+self.height*y_rand[i + self.n_trees + self.n_rocks + self.n_table]) < 0.3:
				self.x_pos.append(-self.width/2 + 0.05)
				self.y_pos.append(-self.height/2 + 0.05)
				p.loadURDF("fire.urdf", basePosition=[-self.width/2+0.05,-self.height/2+0.05, 0], useFixedBase = 1)
			else:
				self.x_pos.append(-self.width/2+self.width*x_rand[i + self.n_trees + self.n_rocks + self.n_table])
				self.y_pos.append(-self.height/2+self.height*y_rand[i + self.n_trees + self.n_rocks + self.n_table])
				p.loadURDF("fire.urdf", basePosition=[-self.width/2+self.width*x_rand[i+self.n_trees+self.n_rocks+self.n_table], -self.height/2+self.height*y_rand[i+self.n_rocks+self.n_trees+self.n_table], 0], useFixedBase = 1)
	...
	def step(self, action):
		...
		if abs(basePos[0] - self.x_pos[self.n_trees + self.n_rocks + self.n_table]) < 0.3 and abs(basePos[1] - self.y_pos[self.n_trees + self.n_rocks + self.n_table]) < 0.3:
	...
	def get_observation(self):
		...
		while True:
			...
			for r in np.arange(0,self.sense_range,0.1):
				...
				for i in range(self.n_trees+self.n_rocks+self.n_table+self.is_fire):
					if x > self.x_low[i] and x < self.x_high[i]:
						if y > self.y_low[i] and y < self.y_high[i]:
							...
							elif i > self.n_trees+self.n_rocks-1 and i<self.n_trees+self.n_rocks+self.n_table:
