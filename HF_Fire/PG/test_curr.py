...
if __name__ == "__main__":
	...
	actionCnt = 5
	D = 103 #90 beams x 4 items lidar + 3 inventory items
	NUM_HIDDEN = 16
	GAMMA = 0.995
	LEARNING_RATE = 1e-3
	DECAY_RATE = 0.99
	...
	for i in range(no_of_environmets):
		...
		env = gym.make(env_id, map_width = width, map_height = height, items_quantity = {'tree': no_trees, 'rock': no_rocks, 'crafting_table': crafting_table, 'stone_axe':0},
			initial_inventory = {'wall': 0, 'tree': starting_trees, 'rock': starting_rocks, 'crafting_table': 0, 'stone_axe':0}, is_fire = is_fire_env, goal_env = type_of_env, is_final = final_status)
