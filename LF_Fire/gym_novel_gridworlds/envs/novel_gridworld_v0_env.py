...
class NovelGridworldV0Env(gym.Env):
    ...
    def __init__(self, map_width=None, map_height=None, items_id=None, items_quantity=None, initial_inventory = None, is_fire = None, goal_env = None, is_final = False):
        ...
        self.items = ['wall', 'tree', 'rock', 'fire', 'crafting_table', 'stone_axe']
        ...
        self.items_quantity = {'tree': 5, 'rock': 2, 'fire': self.is_fire, 'crafting_table': 1, 'stone_axe': 0}
        ...
        self.items_lidar = ['wall', 'crafting_table', 'tree', 'rock', 'fire']
    ...
    def step(self, action):
        ...
        # if self.is_fire == 1:
        #     r,c = self.agent_location
        #     if self.map[r - 1][c] == self.items_id['fire'] or self.map[r + 1][c] == self.items_id['fire'] or self.map[r][c - 1] == self.items_id['fire'] or self.map[r][c + 1] == self.items_id['fire'] \
        #     or self.map[r - 1][c - 1] == self.items_id['fire'] or self.map[r + 1][c + 1] == self.items_id['fire'] or self.map[r - 1][c + 1] == self.items_id['fire'] or self.map[r + 1][c - 1] == self.items_id['fire']:
        #         done = True
        #         reward = self.reward_hit_fire
        #         observation = self.get_observation()
        #         info = {}
