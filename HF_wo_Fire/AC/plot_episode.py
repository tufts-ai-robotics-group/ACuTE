...
for i in range(min_x):
	if curriculum_shifted_timestep_arr[i] < 90000000:
		x_count += 1

for i in range(min_x):
	if final_timestep_arr[i] < 90000000:
		x_count_f += 1


plt.rcParams.update({'font.size': 12})

plt.scatter(curriculum_shifted_timestep_arr[:x_count], curriculum_shifted_reward_arr[:x_count], label = 'Curriculum')
...
plt.scatter(final_timestep_arr[:x_count_f], final_reward_arr[:x_count_f], label = 'Baseline')
